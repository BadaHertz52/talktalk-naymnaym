...
**인라인 style보다 CSS 모듈 클래스를 지향**: 고정된 값(색상 등)은 `index.module.css`에 클래스로 정의해 쓴다. 렌더링 시점에만 계산 가능한 동적 값(반복 렌더에서 인덱스별로 달라지는 `flexGrow`, 조건부 `opacity` 등)에 한해서만 인라인 style을 허용한다.
...
- `return`문 뒤에 다른 코드가 있다면 `return`문 뒤에 빈 줄을 하나 띄운다.
- className을 조건부로 병합할 때는 직접 문자열을 이어붙이지 않고 `clsx`를 사용한다 (예: `clsx(styles.button, variant === 'outline' && styles.outline, className)`)
